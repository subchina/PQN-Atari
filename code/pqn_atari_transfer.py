...
def reinit_layers_if_needed(network, 
                            ...
                            input_shape, 
                            force_reinit_names=()):
    print(f"Force reinit names: {force_reinit_names}")
...
class CNN(nn.Module):
    ...
    @nn.compact
    def _call_(self, x: jnp.ndarray, train: bool):
...
def make_train(config):
    ...
    def train(rng):
        ...
        def create_agent(rng):
            ...
            # Get initial params and batch_stats
            initial_params = network_variables["params"]
            initial_batch_stats = network_variables["batch_stats"]

            # If loading from checkpoint, modify params before creating optimizer
            if "LOAD_PATH" in config and config["LOAD_PATH"] is not None:
                loaded_params, _ = load_model_parameters(config["LOAD_PATH"])
                ...
                if config.get("REINIT_INPUT", False):
                    old_input_channels = loaded_params["CNN_0"]["Conv_0"]["kernel"].shape[2]
                    new_input_channels = input_shape[0]
                    if old_input_channels != new_input_channels:
                        force_reinit.extend(["CNN_0.Conv_0.kernel", "CNN_0.Conv_0.bias"])

                if config.get("REINIT_OUTPUT", False):
                    old_output_dim = loaded_params["Dense_0"]["bias"].shape[0]
                    if old_output_dim != output_dim:
                        force_reinit.extend(["Dense_0.kernel", "Dense_0.bias"])

                rng_for_init = jax.random.PRNGKey(config.get("REINIT_SEED", 999))
                final_params, final_batch_stats = reinit_layers_if_needed(
                    network, loaded_params, rng_for_init, input_shape, tuple(force_reinit)
                )
            else:
                final_params = initial_params
                final_batch_stats = initial_batch_stats

            # Create optimizer with final params
            tx = optax.chain(
            ...
            def _learn_epoch(carry, _):
                ...
                def _learn_phase(carry, minibatch_and_target):
                    ...
                    train_state = train_state.replace(
                        ...
                        batch_stats=freeze(updates["batch_stats"]),
                    )
...
def single_run(config):
    ...
    wandb.init(
        ...
        name=f'{env_name}Transfer_From{config["ENV_NAME_LEARNING"]}',
        config=config,
